...
class ModelEvaluation:
    ...
    def evaluate_model(self) -> EvaluateModelResponse:
        ...
        try:
            ...
            logging.info(f"accuracy_score for this model: {trained_model_accuracy_score}")

            best_model_accuracy_score=None
            best_model = self.get_best_model()
            if best_model is not None:
                logging.info(f"Computing accuracy_score for production model..")
                y_hat_best_model = best_model.predict(x)
                best_model_accuracy_score = accuracy_score(y, y_hat_best_model)
                logging.info(f"accuracy_score-Production Model: {best_model_accuracy_score}, accuracy_score-New Trained Model: {trained_model_accuracy_score}")
            
            tmp_best_model_score = 0 if best_model_accuracy_score is None else best_model_accuracy_score
            result = EvaluateModelResponse(trained_model_accuracy_score=trained_model_accuracy_score,
                                           best_model_accuracy_score=best_model_accuracy_score,
                                           is_model_accepted=trained_model_accuracy_score > tmp_best_model_score,
                                           difference=trained_model_accuracy_score - tmp_best_model_score
                                           )
